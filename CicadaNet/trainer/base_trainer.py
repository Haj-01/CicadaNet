...
from util.metrics import si_sdr
...
class BaseTrainer:
    ...
    def spec_audio_visualization(self, noisy, enhanced, clean, name, epoch):
        # Visualize audio
        self.writer.add_audio(f"Speech/{name}_Noisy", noisy, epoch, sample_rate=32000)
        self.writer.add_audio(f"Speech/{name}_Enhanced", enhanced, epoch, sample_rate=32000)
        self.writer.add_audio(f"Speech/{name}_Clean", clean, epoch, sample_rate=32000)
        ...
        noisy_mag, _ = librosa.magphase(librosa.stft(noisy, n_fft=640, hop_length=320, win_length=640))
        enhanced_mag, _ = librosa.magphase(librosa.stft(enhanced, n_fft=640, hop_length=320, win_length=640))
        clean_mag, _ = librosa.magphase(librosa.stft(clean, n_fft=640, hop_length=320, win_length=640))
        ...
        for k, mag in enumerate([
            ...
            axes[k].set_title(f"mean: {np.mean(mag):.3f}, "
                              ...
                              f"max: {np.max(mag):.3f}, "
                              f"min: {np.min(mag):.3f}")
            librosa.display.specshow(librosa.amplitude_to_db(mag), cmap="magma", y_axis="linear", ax=axes[k], sr=32000)
        plt.tight_layout()
    ...
    def metrics_visualization(self, noisy_list, clean_list, enhanced_list, epoch):
        ...
        batch_num = 0
        sisdr_clean_denoise = 0

        for noisy, clean, enhanced in zip(noisy_list, clean_list, enhanced_list):
            batch_num += 1
            # sisdr_clean_noisy = si_sdr(clean, noisy)
            sisdr = si_sdr(clean, enhanced)
            sisdr_clean_denoise += sisdr

        sisdr_clean_denoise /= batch_num

        self.writer.add_scalars(f"Validation/si-sdr", {
            "clean and enhanced": sisdr_clean_denoise}, epoch)

        print(f"第 {epoch} epoch 的enhanced_sisdr为 {sisdr_clean_denoise}")
        return sisdr_clean_denoise
