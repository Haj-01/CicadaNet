...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for noisy, clean, n_frames_list in self.train_dataloader:
            ...
            noisy_mag = noisy.to(self.device).unsqueeze(1)  # [B, F, T] => [B, 1, F, T]
            # clean = clean.to(self.device).unsqueeze(1)  # [B, F, T] => [B, 1, F, T]
            clean = clean.to(self.device)  # [B, F, T] => [B, F, T]
            # mask = mask.to(self.device)  # [B, F, T] => [B, F, T]

            enhanced_mask = self.model(noisy_mag)  # [B, 1, F, T]

            # enhanced_mask = torch.squeeze(enhanced_mask, dim=1)
            # clean = torch.squeeze(clean, dim=1)
            enhanced_mag = noisy_mag * enhanced_mask
            enhanced_mag = torch.squeeze(enhanced_mag, dim=1)

            loss = self.loss_function(enhanced_mag, clean, n_frames_list, self.device)
            loss.backward()
            self.optimizer.step()
    ...
    def _validation_epoch(self, epoch):
        ...
        for i, (noisy, clean, name) in tqdm(enumerate(self.validation_dataloader), desc="Inference"):
            ...
            # mask = np.sqrt(clean_mag ** 2 / noisy_mag ** 2)
            ...
            clean_mag = torch.tensor(clean_mag, device=self.device)[None, :, :]     # [F, T] => [1, F, T]
            # mask = torch.tensor(mask, device=self.device)[None, :, :]  # [F, T] => [1, F, T]

            enhanced_mask = self.model(noisy_mag)
            enhanced_mag = noisy_mag * enhanced_mask
