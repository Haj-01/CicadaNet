...
class Uformer(nn.Module):
    ...
    def forward(self, inputs, src):
        ...
        print(f"input_out: {out.shape}")
        out = out[:, :, 1:]
        mag = mag[:, :, 1:]
        encoder_out = []
        mag_out = []
        print(f"input_out-1: {out.shape}")
        ...
        print(f"encoder_out: {out.shape}")
        out, mag = self.conformer(out, mag)
        print(f"conformer_out: {out.shape}")
        ...
        output_cplx = torch.stack([output_real, output_imag], 1) # N x 2 x F x T
